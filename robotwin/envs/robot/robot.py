...
import sapien.core as sapien
from robotwin.assets import load_curobo_config
from robotwin.envs import _GLOBAL_CONFIGS as CONFIGS
...
class Robot:
    ...
    def _init_robot_(self, scene, need_topp=False, **kwargs):
        ...
        self.left_robot_file = kwargs["left_robot_file"]
        self.right_robot_file = kwargs["right_robot_file"]

        self.need_topp = need_topp

        self.left_urdf_path = os.path.join(
            self.left_robot_file, left_embodiment_args["urdf_path"]
        )
        self.left_srdf_path = left_embodiment_args.get("srdf_path", None)
        if self.left_srdf_path is not None:
            self.left_srdf_path = os.path.join(
                self.left_robot_file, self.left_srdf_path
            )
        ...
        self.right_urdf_path = os.path.join(
            self.right_robot_file, right_embodiment_args["urdf_path"]
        )
        self.right_srdf_path = right_embodiment_args.get("srdf_path", None)
        if self.right_srdf_path is not None:
            self.right_srdf_path = os.path.join(
                self.right_robot_file, self.right_srdf_path
            )
    ...
    def set_planner(self, scene=None):
        left_curobo_config = load_curobo_config(
            self.left_robot_file, "left"
        )
        right_curobo_config = load_curobo_config(
            self.right_robot_file, "right"
        )

        self.communication_flag = self.left_robot_file != self.right_robot_file
        ...
        if not self.communication_flag:
            ...
            if self.planner_backend == "curobo":
                ...
                self.left_planner = CuroboPlanner(self.left_entity_origion_pose,
                                                ...
                                                robot_config=left_curobo_config)
                self.right_planner = CuroboPlanner(self.right_entity_origion_pose,
                                                ...
                                                robot_config=right_curobo_config)

...
def _assets_root() -> Path:
    configured = os.getenv("ROBOTWIN_ASSETS_ROOT") or os.getenv("ASSETS_PATH")
    if not configured:
        raise RuntimeError(
            "ROBOTWIN_ASSETS_ROOT is required to load RoboTwin object assets"
        )
    return Path(configured).expanduser().resolve()


@lru_cache(maxsize=None)
def _load_cluttered_objects(assets_root: str):
    base_dir = Path(assets_root)
    ...
    cluttered_objects_config = json.load(
        open(
            base_dir / "assets/objects/objaverse/list.json",
            "r",
            encoding="utf-8",
        )
    )
    ...
    same_obj = json.load(
        open(base_dir / "assets/objects/same.json", "r", encoding="utf-8")
    )
...
def get_all_cluttered_objects():
    return _load_cluttered_objects(str(_assets_root()))


def get_available_cluttered_objects(entity_on_scene: list):
    cluttered_objects_info, cluttered_objects_list, same_obj = (
...
def create_cluttered_urdf_obj(scene, pose: sapien.Pose, modelname: str, scale=1.0, fix_root_link=True) -> Actor:
    ...
    modeldir = _assets_root() / "assets" / modelname
